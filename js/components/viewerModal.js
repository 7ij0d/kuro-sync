...
function openItemViewer(itemId) {
  ...
  if (item.type === 'text' || item.type === 'clipboard' || item.type === 'note') {
    ...
    const fileUrl = item.file_path ? `/api/items/${item.id}/file` : '';
    bodyEl.innerHTML = `
      <div style="display:flex; flex-direction:column; align-items:center; gap:16px;">
        <div style="max-height:460px; overflow:hidden; border-radius:var(--radius-lg); border:1px solid var(--border-subtle); background:var(--bg-surface-subtle); display:flex; align-items:center; justify-content:center;">
          <img src="${fileUrl}" alt="${item.title}" style="max-width:100%; max-height:460px; object-fit:contain;" />
        </div>
        <div style="display:flex; gap:12px;">
          <button class="btn-card-action" onclick="copyCardImage('${item.id}', this)">
            ${window.i18n ? window.i18n.t('copyImage') : 'Copy Image'}
          </button>
          <button class="btn-primary" onclick="downloadItemFile('${item.id}')">
            ${window.i18n ? window.i18n.t('download') : 'Download'}
          </button>
        </div>
      </div>
    `;
    ...
    if (isPdf) {
      bodyEl.innerHTML = `
        <div style="display:flex; flex-direction:column; gap:14px;">
          <iframe src="${fileUrl}#toolbar=0" style="width:100%; height:450px; border-radius:var(--radius-md); border:1px solid var(--border-subtle);" frameborder="0"></iframe>
          <div style="display:flex; justify-content:flex-end; gap:12px;">
            <button class="btn-secondary" onclick="window.open('${fileUrl}', '_blank')">${window.i18n ? window.i18n.t('open') : 'Open in New Tab'}</button>
            <button class="btn-primary" onclick="downloadItemFile('${item.id}')">${window.i18n ? window.i18n.t('download') : 'Download'}</button>
          </div>
        </div>
      `;
    } else {
    ...
    bodyEl.innerHTML = `
      <div style="display:flex; flex-direction:column; gap:16px; padding:12px 0;">
        ...
        <div style="display:flex; justify-content:flex-end; gap:12px;">
          <button class="btn-secondary" onclick="window.clipboardEngine.copyText('${escapeHtml(item.content)}')">${window.i18n ? window.i18n.t('copy') : 'Copy Link'}</button>
          <button class="btn-primary" onclick="window.open('${escapeHtml(item.content)}', '_blank')">${window.i18n ? window.i18n.t('openLink') : 'Open Link'}</button>
        </div>
      </div>
    `;
  }
